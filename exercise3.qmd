...
```{r}
d <- d %>% mutate(diet_strategy=case_when(Fruit>=50 ~ "frugivore", Leaves>=50 ~ "folivore", Fruit<50&Leaves<50~"omnivore"))%>% relocate(Scientific_Name, Family, Genus, Species, diet_strategy)
View(d)
```
#### Boxplots
```{r}
